...
def getVictoryPoints(ts):
   ...
   fnames_for_scoring = ['team','runs_for','runs_against','errors','attendance','total']
...
def makeTeamDict(tname,rfteam,rateam,errteam,m1,m2,m3,m4):
   dd = {}
   dd['team_name'] = tname
   dd['runs_for_team'] = rfteam
   dd['runs_against_team'] = rateam
   dd['error_team'] = errteam
   dd['runs_for'] = 0
   dd['runs_against'] = 0
   dd['errors'] = 0
   dd['attendance'] = 0
...
def getFilledTeams(date1,date2):
   ...
   for t in ts:
      t['runs_for'] = sum([x['runs_for'] for x in ress if x['team'] == t['runs_for_team']])
      t['runs_against'] = -sum([x['runs_against'] for x in ress if x['team'] == t['runs_against_team']])
      t['errors'] = -sum([x['errors'] for x in ress if x['team'] == t['error_team']])
      att = sum([x['attendance'] for x in ress if x['team'] == t['error_team']])
      att = att+ sum([x['attendance'] for x in ress if x['team'] == t['runs_against_team']])
      att = att+sum([x['attendance'] for x in ress if x['team'] == t['runs_for_team']])
      t['attendance'] = att
   return ts
...
def OutputTablesToFile(filename,ts,tsToday):
   tls = ts
...
#   m5scores = scoreVector(x['total'] for x in svps)
   ...
   for svi in svps:
#      m5s = m5scores[ii]
      m5s = scoreSingle([x['total'] for x in svps],ii)      
   ...
   printDictList(ff,sts,['team_name','runs_for','runs_against','errors','attendance'])
   ff.flush()
   ff.write('<BR><BR>Points<BR>')
   printDictList(ff,svps,['team_name','runs_for','runs_against','errors','attendance','total'])
...
def DoTheDay():
   ...
   tsToday = getFilledTeams(d2s(end_date),d2s(end_date))
   OutputTablesToFile('/home/eddie7/code/wb3m5/stats_wb3m5.html',ts,tsToday)
