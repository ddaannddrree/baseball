...
def countGames(date1,date2, do='both'):
   res = Counter()
   ...
   while thedate <=pdate2:
      ...
      glist = DateGames(thedate.strftime('%Y_%m_%d'))
      if do=='both' or do=='aways':
         aways = [str.rsplit(str.rsplit(str(x),'_')[3],'mlb')[0] for x in glist]
         res.update(aways)
      if do=='both' or do=='homes':
         homes = [str.rsplit(str.rsplit(str(x),'_')[4],'mlb')[0] for x in glist]
         res.update(homes)
