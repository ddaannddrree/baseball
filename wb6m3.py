...
def ExtractTeamRecords(dd,curdate):
   ...
   try:
      ...
      tgh['runs_against'] = int(dd['data']['boxscore']['linescore']['away_team_runs'])
      tga['runs_against'] = int(dd['data']['boxscore']['linescore']['home_team_runs'])
...
def CompileDayGames(curdate):
   ...
   for g in gameids:
      ...
      if not jsonbox is None:
         ...
         if jsonbox['data']['boxscore']['status_ind'].startswith('F'):
             trs = ExtractTeamRecords(jsonbox,curdate)
             if not trs is None:
...
def scoreBowlingFrame(fnum,t) :
   ...
   if fnum < 9:
      ...
      if p1==10 and p2==10:
         t['frames'][fnum]['total'] = getPreviousScore(fnum,t)+20+t['frames'][fnum]['p3']
      elif p1==10:
         t['frames'][fnum]['total'] = getPreviousScore(fnum,t)+10+p2+t['frames'][fnum]['p3']
      elif p1+p2==10:
         t['frames'][fnum]['total'] = getPreviousScore(fnum,t)+10+t['frames'][fnum]['p3']
      else:
         t['frames'][fnum]['total'] = getPreviousScore(fnum,t)+p1+p2
...
def fillFramesForTeam(t,ress):
   ...
   elif (fnum > 9) and (f10['p1']+f10['p2'])==10: #, do nothing
      if f10['p1']==10 and f10['p2']==10 or f10['p1']!=10 and f10['p1']+f10['p2']==10:
         f10['p3'] = pinnize(f11['r1']+f11['r2'])
      else:   
         f10['p3'] = pinnize(max(f11['r1'],f11['r2']))
...
def getRangeGameIDs(d1,d2):
    res = []
    pdate1 = datetime.strptime(d1,'%Y_%m_%d').date()
    pdate2 = datetime.strptime(d2,'%Y_%m_%d').date()
    if pdate2 < pdate1:
        raise Exception('date2 must be at or after date1')
    oneday = timedelta(1)
    thedate = pdate1
    while thedate <= pdate2:
        sys.stdout.flush()
        (rs) = DateGames(thedate.strftime('%Y_%m_%d'))
        if len(rs) > 0:
            res.extend(rs)
        thedate = thedate+oneday
    return (res)  
...
def DoTheDay():
    ...
    pickle.dump( ress_16, open( "ress_16_wb6m3.p", "wb" ) )
    ress_17 = getFilledTeams(d2s(start_date_17),d2s(end_date_17))
    pickle.dump( ress_17, open( "ress_17_wb6m3.p", "wb" ) )
...
def doCrap(ress,mlb):
    runsag17 = {tt:numpy.mean([r['runs_against'] for r in ress if r['team'] == tt and '2017' in r['date']]) for tt in mlb }
    runs17 = {tt:numpy.mean([r['runs'] for r in ress if r['team'] == tt and '2017' in r['date']]) for tt in mlb }
    med_runs17 = {tt:numpy.median([r['runs'] for r in ress if r['team'] == tt and '2017' in r['date']]) for tt in mlb }
    med_runsag17 = {tt:numpy.median([r['runs_against'] for r in ress if r['team'] == tt and '2017' in r['date']]) for tt in mlb }
    perc_runs17 = {tt:numpy.mean([bhit(r['runs']) for r in ress if r['team'] == tt and '2017' in r['date']]) for tt in mlb }
    perc_runsag17 = {tt:numpy.mean([phit(r['runs_against']) for r in ress if r['team'] == tt and '2017' in r['date']]) for tt in mlb }
    print 'team,r_mean,r_median,rag_mean,rag_median,r_perc_above,r_per_below'
    for tt in mlb:
        print tt+','+ str(runs17[tt])+','+str(runsag17[tt])+','+str(med_runs17[tt])+','+str(med_runsag17[tt])+','+str(perc_runs17[tt])+','+str(perc_runsag17[tt])
