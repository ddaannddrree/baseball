...
def getVictoryPoints(ts):
   ...
   fnames_for_scoring = ['team','wins','total']
...
def getFilledTeams(date1,date2):
   ...
   for t in ts:
      ...
      for tn in ['t1','t2','t3','t4']:
         wstr = tn+'w'
         if (isinstance(t[tn],int)):
            w = w + t[tn]
            t[wstr] = t[tn]
         else:
            t[wstr] = sum([x['win'] for x in ress if x['team'] == t[tn]])
            if (t[tn] == 'cle'):
               t[wstr] = t[wstr] + 0.5
            w = w + t[wstr]
      t['wins'] = w
...
def getTeams():
   ...
   ts.append(makeTeamDict(team_names[0],['chn','cin','lad'],['cle','mil','was'],4,4)
   ts.append(makeTeamDict(team_names[1],['ari','mil','sdn'],['cha','laa','stl'],5.5,1)
   ts.append(makeTeamDict(team_names[2],['col','hou','sfn'],['chn','nya','sfn'],5.5,5)
   ts.append(makeTeamDict(team_names[3],['mia','oak','was'],['cin','pit','sea'],3,2)
   ts.append(makeTeamDict(team_names[4],['kca','pit','stl'],['hou','kca','tex'],1,6)
   ts.append(makeTeamDict(team_names[5],['bos','cle','tba'],['ari','nyn','sdn'],7,8)
   ts.append(makeTeamDict(team_names[6],['det','min','tor'],['lad','mia','oak'],2,3)
   ts.append(makeTeamDict(team_names[7],['atl','bal','tex'],['atl','det','tba'],8,7)
   return ts
           
def getTeamsWB3M5():
   ts = [];
   team_names = ['Pierzinskis Misspelled D-Bags','No-Talent Ass Clowns', 'Portlandia Misfits', 'The Rube', 'Paly Players', 'Dr. Watson', 'Buena Vista Bridesmaids', 'Damnedest of the Nice']
   ts.append(makeTeamDict(team_names[0],'oak','sfn','ana',8,7,8,4))
   ts.append(makeTeamDict(team_names[1],'det','oak','sfn',6,5,5,6))
   ts.append(makeTeamDict(team_names[2],'mil','sln','nya',3,4,1,1))
   ts.append(makeTeamDict(team_names[3],'tor','was','sln',4,3,3,3))
   ts.append(makeTeamDict(team_names[4],'col','lan','bos',1,1,6.5,2))
   ts.append(makeTeamDict(team_names[5],'bos','ana','lan',5,8,6.5,7))
   ts.append(makeTeamDict(team_names[6],'lan','cin','bal',7,6,2,5))
   ts.append(makeTeamDict(team_names[7],'ana','sdn','chn',2,2,4,8))
   return ts
...
def ExtractGameScores(dd):
   res = []
   resN = {}
   try:
      resN['home_team'] = dd['data']['boxscore']['home_team_code']
      resN['home_runs'] = int(dd['data']['boxscore']['linescore']['home_team_runs'])
      resN['away_team'] = dd['data']['boxscore']['away_team_code']
      resN['away_runs'] = int(dd['data']['boxscore']['linescore']['away_team_runs'])
      ils = dd['data']['boxscore']['linescore']['inning_line_score']
...
#      resH['errors'] = int(dd['data']['boxscore']['linescore']['home_team_errors'])
#      resH['date'] = dd['data']['boxscore']['date']
#      resH['game_id'] = dd['data']['boxscore']['game_id']
#      resH['attendance'] = attendance
 ...
 #     resA['errors'] = int(dd['data']['boxscore']['linescore']['away_team_errors'])
 #     resA['date'] = dd['data']['boxscore']['date']
 #     resA['game_id'] = dd['data']['boxscore']['game_id']
 #     resA['attendance'] = attendance
      if (resH['runs_for'] > resA['runs_for']):
         resN['team'] = dd['data']['boxscore']['home_team_code']
         resN['win'] = 1
         resN['game_id'] = dd['data']['boxscore']['game_id']
         res.append(resN)
      else:
         if (resH['runs_for'] < resA['runs_for']):
            resN['team'] = dd['data']['boxscore']['away_team_code']
            resN['win'] = 1
            resN['game_id'] = dd['data']['boxscore']['game_id']
            res.append(resN)
   except:
...
def OutputTablesToFile(filename,ts,gs):
   tls = ts
...
#   m5scores = scoreVector(x['total'] for x in svps)
   ii = 0
   for svi in svps:
#      m5s = m5scores[ii]
      m6s = scoreSingle([x['total'] for x in svps],ii)      
      svi['through_six'] = svi['mtotal'] + m6s
      svi['m6'] = m6s
      ii = ii+1
   ff = open(filename,'wb')
#   ff.write('teams<br>')
#   printDictList(ff,sts,['team_name','t1','t2','t3','t4'])
#   ff.flush()
   ff.write('<BR><BR>Stats<BR>')
   printDictList(ff,sts,['team_name','t1','t1w','t2','t2w','t3','t3w','t4','t4w','wins'])
   ff.flush()
   ff.write('<BR><BR>Points<BR>')
   printDictList(ff,svps,['team_name','wins','total'])
#   ff.write('<BR><BR>Today Stats<BR>')
#   printDictList(ff,stsToday,['team_name','runs_for','runs_against','errors','attendance'])
   ff.write('<BR><BR>Season scores As of Today:<BR>')

   ssvps = sorted(svps,key=lambda k: k['through_six'],reverse=True)
   printDictList(ff,ssvps,['team_name','m1','m2','m3','m4','m5','m6','through_six'])
   ff.write('<BR><BR>')

   ff.write('Today''s Games<BR>')

   printDictList(ff,gs,['inning','home_team','away_team','home_runs','away_runs'])
   ff.write('<BR><BR>')
...
def DoTheDay():
   ...
   end_date = date(2014,11,10)
   end_date = min(end_date,today)
   ts = getFilledTeams(d2s(start_date),d2s(end_date))
   gs = CompileDayScores(d2s(today))
   OutputTablesToFile('/home/eddie7/code/wb3m6/stats_wb3m6.html',ts,gs)
...
def CompileDayGames(date):
   ...
   for g in gameids:
      print 'Doing game ' + g
      res.extend(ExtractTeamRecords(GetGameBoxScoreJson(g)))
   return res

def CompileDayScores(date):
   res = []
   gameids = DateGames(date)
   for g in gameids:
      print 'Doing game ' + g
      res.extend(ExtractGameScores(GetGameBoxScoreJson(g)))
   return res
