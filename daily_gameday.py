...
def getVictoryPoints(ts):
   ...
   fnames_for_scoring = ['team','H','BB','ASS','HR','SB','QS','ER','DP','W','SV','team_name_streak','total']
...
def OutputTablesToFile(filename):
   ts = getTeamScores()
   tls = getTeamScores(True)
   ...
   stls = [x for (x,y) in sorted(zip(tls,vps),key=lambda k: k[1]['total'],reverse=True)]   
   ff = open(filename,'wb')
   ff.write('Stats<BR>')
   printDictList(ff,sts,['team_name','H','BB','ASS','HR','SB','QS','ER','DP','W','SV','team_name_streak'])
   ff.flush()
   ff.write('<BR><BR>Points<BR>')
   printDictList(ff,svps,['team_name','H','BB','ASS','HR','SB','QS','ER','DP','W','SV','team_name_streak','total'])
   ff.write('<BR><BR>Active Streaks<BR>')
   printDictList(ff,stls,['team_name','H','BB','ASS','HR','SB','QS','ER','DP','W','SV','team_name_streak'])
   ff.flush()
   ff.write("<BR><BR>Here is a <a href='all.csv'> link </a> to the csv file with all the data. <BR>Last updated: ")
...
def DoTheDay():
   ...
   RangeGamesToDB(d2s(yesterday),d2s(today))
   OutputTablesToFile('/home/eddie7/code/wb3m4/stats.html')
   printTeamsCSV('/home/eddie7/code/wb3m4/all.csv')
